...
if 'useNavigate' not in text:
    text = text.replace("import { RiskBadge }", "import { useNavigate } from 'react-router-dom';\nimport { apiService } from '../../api/client';\nimport { RiskBadge }")

needed_icons = ['HelpCircle', 'ChevronDown', 'ChevronUp', 'Cpu', 'AlertCircle']
for icon in needed_icons:
    if icon not in text:
        text = text.replace("import { X, ", f"import {{ X, {icon}, ")

# Add state variables inside ExplainSheet
if 'isHowCalculatedOpen' not in text:
    text = text.replace(
        "if (!isOpen || !anomalyItem) return null;",
        "const [isHowCalculatedOpen, setIsHowCalculatedOpen] = useState(false);\n  const [creatingCase, setCreatingCase] = useState(false);\n  const navigate = useNavigate();\n\n  if (!isOpen || !anomalyItem) return null;"
    )

# Case initiation handler
new_handler = '''  const handleInitiateCase = async () => {
...
if 'handleInitiateCase' not in text:
    text = text.replace("if (!isOpen || !anomalyItem) return null;", new_handler + "\n\n  if (!isOpen || !anomalyItem) return null;")

# Footer button replacement
text = text.replace(
    "onClick={() => onInitiateCase && onInitiateCase(anomalyItem)}",
    "onClick={handleInitiateCase} disabled={creatingCase}"
)

# Insert Section 1 and Section 2
sections_jsx = '''
            {/* ========================================================================= */}
            ...
            <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 font-mono text-[11px]">
                ...
                <div className="p-2.5 rounded-lg bg-slate-900 border border-slate-800">
                  ...
                  <span className="font-bold text-slate-200">{featureValues.peer_median ? ₹ : 'Not available'}</span>
                </div>
                <div className="p-2.5 rounded-lg bg-slate-900 border border-slate-800">
...
if 'HOW AI CALCULATED THIS RISK' not in text:
    text = text.replace('{/* Verification Checklist */}', sections_jsx + '\n\n            {/* Verification Checklist */}')

with open('d:/NidhiDristi/frontend/src/components/risks/ExplainSheet.jsx', 'w', encoding='utf-8') as f:
    f.write(text)

print('ExplainSheet updated with Section 1 and Section 2')